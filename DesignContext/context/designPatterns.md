...
## Cache-Aside — Consulta de catálogo de productos

**Funcionalidad**: Al escanear un código de barras, el sistema obtiene la información del producto consultando primero el caché. Si el dato no existe o expiró, lo recupera desde la base de datos, lo almacena en caché y retorna el resultado.

**Actores**:

* `ScanController` (Client): solicita la información del producto después del escaneo.
* `ProductCacheService` (Cache Manager): implementa la lógica Cache-Aside.
* `RedisCacheStore` (Cache): almacena temporalmente los productos consultados.
* `ProductRepository` (Data Source): obtiene los datos desde PostgreSQL cuando ocurre un cache miss.
* `ProductDTO` (Data Object): representa la información retornada al cliente.

**Por qué**: reduce la carga sobre la base de datos y mejora el tiempo de respuesta durante el escaneo de productos. Además, permite mantener disponibilidad mediante fallback a la BD cuando el caché no está disponible.

---

## Human-in-the-Loop (HITL) — Validación de fraude en generación de QR

**Funcionalidad**: Antes de validar una sesión de compra mediante QR, un agente de AI analiza el riesgo de fraude. Los casos de bajo y medio riesgo se aprueban automáticamente, mientras que los de alto riesgo son enviados a revisión humana antes de tomar una decisión.

**Actores**:

* `ValidationOrchestrator` (Coordinator): coordina el flujo completo de validación.
* `FraudDetectionAgent` (AI Evaluator): analiza métricas de la sesión y calcula el nivel de riesgo.
* `HumanReviewQueue` (Review Queue): almacena los casos que requieren intervención humana.
* `Review Dashboard` (Human Reviewer Interface): permite al operador revisar y aprobar o rechazar sesiones.
* `Backoffice Operator` (Human Reviewer): toma la decisión final en casos de alto riesgo.
* `PushNotificationService` (Notifier): notifica a los revisores cuando una sesión requiere atención.
* `ValidationResult` (Outcome): comunica el resultado final de la validación al sistema.

**Por qué**: permite aprovechar la velocidad y escalabilidad de la AI para la mayoría de los casos, manteniendo supervisión humana en decisiones sensibles donde un falso positivo o falso negativo podría afectar la experiencia del usuario o generar fraude.
